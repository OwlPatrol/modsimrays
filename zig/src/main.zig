...
pub fn main() !void {
   ...
   // Camera Values
    ...
    switch(3) {
        ...
        2 => {
            try Utils.twoSpheres(&scene);
            look_from = point(13, 2, 3);
            look_at = point(0, 0, 0);
            vfov = 20.0;
        },
        3 => {
            try Utils.twoPerlinSpheres(&scene);
            look_from = point(13,2,3);
            look_at = point(0,0,0);
            vfov = 20.0;
        },
    ...
    var tree = try Bvh.init(0, 1, &scene);
    ...
    for (0..height) |row| {
        ...
        for (0..width) |col| {
            ...
            for (0..samples) |_| {
                const u = (@intToFloat(f64, col) + Utils.floatRand(0, 1)) / @intToFloat(f64, width - 1);
                const v = (@intToFloat(f64, height - row) + Utils.floatRand(0, 1)) / @intToFloat(f64, height - 1);
